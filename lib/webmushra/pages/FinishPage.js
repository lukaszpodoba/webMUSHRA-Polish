...
FinishPage.prototype.render = function (_parent) {
    ...
    for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      if (element.type === "text") {
        var valueAttribute = element.default ? "value='" + element.default + "'" : "";
        table.append($("<tr><td><strong>"+ element.label +"</strong></td><td><input id='"+element.name+"' " + valueAttribute + " /></td></tr>"));
      } else if (element.type === "number") {
        var valueAttribute = element.default ? "value='" + element.default + "'" : "";
        table.append($("<tr><td><strong>"+ element.label +"</strong></td><td><input id='"+element.name+"' min='"+element.min+"' max='"+element.max+"' " + valueAttribute + " data-inline='true'/></td></tr>"));
      } else if (element.type === "select") {
        var options = "";
        for (var j = 0; j < element.options.length; j++) {
          options += "<option value='" + element.options[j] + "'>" + element.options[j] + "</option>";
        }
        table.append($("<tr><td><strong>" + element.label + "</strong></td><td><select id='" + element.name + "'>" + options + "</select></td></tr>"));
