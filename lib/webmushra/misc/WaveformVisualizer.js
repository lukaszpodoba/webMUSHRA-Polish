...
function WaveformVisualizer(_variableName, _parent, _stimulus, _showWaveform, _enableLooping, _mushraAudioControl){
  ...
  this.rightRegionPosition = this.resampledSamples.length; // samples
    ...
    this.numberEventListener = this.mushraAudioControl.addEventListener((function (_event) {
      if (_event.name == 'processUpdate') {
      ...
      } else if (_event.name == 'stimulusChanged') {
        this.stimulus = _event.stimulus;
        var audioBuffer = this.stimulus.getAudioBuffer();
        var numberOfChannels = audioBuffer.numberOfChannels;
        this.avgOriginalSamples = new Float32Array(audioBuffer.length);
        for(var i = 0; i < audioBuffer.length; ++i) {
          var sum = 0;
        ...
        this.slider.get(0).noUiSlider.updateOptions({
          range: {
            'min': 0,
            'max': _event.stimulus.audioBuffer.length
          }
        });
        this.slider.get(0).noUiSlider.set([0, _event.stimulus.audioBuffer.length]);
      }
...
WaveformVisualizer.prototype.renderTable = function() {
  ...
  // var upperLim = $("<div id='div_upper_limit'><input type=\"text\" name=\"name\" id=\"upperLim\" class=\"limits\" data-inline=\"true\" data-mini=\"true\"disabled=\"disabled\"></input></div>");  // td11_.append(lowerLim);
